...
if(1==1)
  ...
  vid=netcdf.inqVarID(ncid0,'ZETA_SETUP'); %(nvrt,np,ntime)
  % vid=netcdf.inqVarID(ncid0,'HS');
...
% globally max zeta_setup
  size_s = size(S);
  time_fram_num = size_s(2);
  global_max = zeros([size_s(1),1]);
  for titem = 1:time_fram_num
    for ielement = 1:size_s(1)
        if S(ielement,titem)>global_max(ielement)
            global_max(ielement) = S(ielement,titem);
        end
    end
  end
...
%   ncid0 = netcdf.open('./wwm_hist_0002.nc','NC_NOWRITE');
%   dimid = netcdf.inqDimID(ncid0,'mnp');
%   [~,np] = netcdf.inqDim(ncid0,dimid);
%   dimid = 2;
%   [~,nvrt] = netcdf.inqDim(ncid0,dimid);
...
%   netcdf.close(ncid0);
% 
% ncid0 = netcdf.open('./wwm_hist_0003.nc','NC_NOWRITE');
%   dimid = netcdf.inqDimID(ncid0,'mnp');
%   [~,np] = netcdf.inqDim(ncid0,dimid);
%   dimid = 2;
%   [~,nvrt] = netcdf.inqDim(ncid0,dimid);
...
%   netcdf.close(ncid0);
% 
%   ncid0 = netcdf.open('./wwm_hist_0004.nc','NC_NOWRITE');
%   dimid = netcdf.inqDimID(ncid0,'mnp');
%   [~,np] = netcdf.inqDim(ncid0,dimid);
%   dimid = 2;
%   [~,nvrt] = netcdf.inqDim(ncid0,dimid);
